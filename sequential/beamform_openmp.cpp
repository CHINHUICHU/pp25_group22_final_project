// beamform_openmp.cpp - OpenMP parallelized version
#include "beamform.h"
    ...
    const char* beamfile)
{
    ...
    cout << "===== OpenMP Beamforming Parameters =====\n";
    cout << "Nchan      = " << Nchan      << "\n";
    cout << "Nbeam      = " << Nbeam      << "\n";
    ...
    if (!fout) {
        cerr << "[Beamform OpenMP] Cannot open output beam file: " << beamfile << endl;
        exit(1);
    }
    ...
    for (int beam = 0; beam < Nbeam; beam++)
    {
        ...
        vector<float> beamsum(UNsample, 0.0f);
        ...
        // Parallelize over TX channels
        #pragma omp parallel
        {
            // Thread-local buffers
            vector<float> buff(Nsample + 8);
            vector<float> buff2(UNsample + 2);
            vector<float> local_beamsum(UNsample, 0.0f);
                ...
                for (int rx = 0; rx < Nchan; rx++)
                {
                    ...
                    for (int j = 0; j < UNsample; j++)
                    {
                        ...
                        if (idx >= 0 && idx < UNsample)
                            local_beamsum[j] += buff2[idx];
                    }
                }
            }

            // Reduce local results into global beamsum
            #pragma omp critical
            {
                for (int j = 0; j < UNsample; j++)
                    beamsum[j] += local_beamsum[j];
            }
        }
    ...
    cout << "================ OpenMP Beamforming Finished ================\n";
    cout << "Total time = "
